...
class BatchDataBLL:
        ...
        for ImIndex,ImRow in ImDf.iterrows():
            for ColIndex,ColRow in ColDf.iterrows():
                if((ImRow["LineID"]==ColRow["LineID"] )&
                    ...
                    jsonStr = ImRow["GroupParameterTag"] + "||" + startTime + "||" + endTime + "||" + SelectType + "||" + str(PL);

                    hisDf = WebSocketHelp.WebSocketJson("ws://10.130.65.207:8181/HisWeb",jsonStr)

                    if(hisDf is not None):
                        if(len(hisDf) > 0):
                            HisEy = HisEntity(ImRow["LineName"], ImRow["StageName"], ImRow["ProcessName"],ImRow["ParameterName"], hisDf)
                            DfList.append(HisEy)
        # 数据整理
        #print(DfList)
        AllDf = None
        ...
        for ImIndex,ImRow in ImDf.iterrows():
            for ColIndex,ColRow in ColDf.iterrows():
                if((ImRow["LineID"]==ColRow["LineID"] )&
                    ...
                    HisEy = HisEntity(ImRow["LineName"], ImRow["StageName"], ImRow["ProcessName"],ImRow["ParameterName"], None)
    ...
    def GetBatchByGroupCode(self,GroupBatchCode):
        ms = Sql200()
        df = ms.ExecQuery("select * from V_SilkBatch where 1=1 and CraftType='006cfd06-c513-4f7c-a9b5-80e3218bff02' and GroupBatchCode='"+GroupBatchCode+"'")
        return df
    ...
    def GetImpParameter(self):
        ...
        ms = Sql200()
        df = ms.ExecQuery("select ID,FactoryCode,FactoryName,ZoneCode,ZoneName,BrandCode,BrandName,LineID,LineName,StageID,StageName,ProcessID,ProcessName,ParameterID,ParameterName,"+
    ...
    def GetBatchTime(self,ParameterComputerID):
        # "0019de43-1a5c-4463-ac2a-ba64f1ff18a0"
        ms = Sql200()
        df = ms.ExecQuery("SELECT a.IndexComputerID, a.IndexValue, a.IndexID, a.ParameterComputerID, c.IndexName 'IndexName', c.Countnumber, c.Sort, c.FormulaCode "+
